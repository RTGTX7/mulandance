...
export default function ClassroomRequestPage() {
  ...
  function buildNotes() {
    return [
      `${t('classroomsPage.basicNotesLabel')}: ${basic?.notes || '-'}`,
      `${t('classroomsPage.attendeesLabel')}: ${details.attendees || '-'}`,
      `${t('classroomsPage.activityDescriptionLabel')}: ${details.activity_description || '-'}`,
      `${t('classroomsPage.equipmentNeedsLabel')}: ${details.equipment_needs || '-'}`,
      `${t('classroomsPage.specialRequestsLabel')}: ${details.special_requests || '-'}`,
      `${t('classroomsPage.additionalNotesLabel')}: ${details.notes || '-'}`,
    ].join('\n');
  }
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!basic) {
      setError(t('classroomsPage.basicMissingError'));
      return;
    }
    ...
    try {
      if (!details.attendees.trim() || !details.activity_description.trim()) {
        setError(t('classroomsPage.missingDetailFields'));
        return;
      }
      const conflict = findTimeConflict(confirmedBookings, basic);
      if (conflict) {
        setError(
          interpolate(t('classroomsPage.timeConflictError'), {
            room: roomLabels[basic.room as ClassroomRoom],
            day: weekdays[basic.day_of_week],
    ...
    <div className="min-h-screen bg-slate-100 pt-16">
      <main className="container py-8">
        <div className="mb-5 flex items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-bold text-slate-950">{t('classroomsPage.detailTitle')}</h1>
            <p className="mt-1 text-sm text-slate-500">
              {t('classroomsPage.detailIntro')}
            </p>
          </div>
          <Button asChild variant="outline">
            <Link href={`/${locale}/classrooms#book`}>
              <ArrowLeft className="mr-2 h-4 w-4" />
              {t('classroomsPage.backToBasic')}
            </Link>
          </Button>
        </div>
        ...
        {!basic ? (
          ...
          <div className="grid gap-6 lg:grid-cols-[0.8fr_1.2fr]">
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-lg">
                  ...
                  {t('classroomsPage.basicInfoTitle')}
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-3 text-sm">
                <div className="rounded-md border bg-slate-50 p-3">
                  <div className="text-slate-500">{t('classroomsPage.room')}</div>
                  <div className="font-medium text-slate-950">{roomLabels[basic.room as ClassroomRoom]}</div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-md border bg-slate-50 p-3">
                    <div className="text-slate-500">{t('classroomsPage.weekday')}</div>
                    <div className="font-medium text-slate-950">{weekdays[basic.day_of_week]}</div>
                  </div>
                  <div className="rounded-md border bg-slate-50 p-3">
                    <div className="text-slate-500">{t('classroomsPage.time')}</div>
                ...
                <div className="rounded-md border bg-slate-50 p-3">
                  <div className="text-slate-500">{t('classroomsPage.purpose')}</div>
                  <div className="font-medium text-slate-950">{basic.title}</div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded-md border bg-slate-50 p-3">
                    <div className="text-slate-500">{t('classroomsPage.applicant')}</div>
                    <div className="font-medium text-slate-950">{basic.applicant_name}</div>
                  </div>
                  <div className="rounded-md border bg-slate-50 p-3">
                    <div className="text-slate-500">{t('classroomsPage.contact')}</div>
            ...
            <Card>
              <CardHeader>
                <CardTitle>{t('classroomsPage.detailFormTitle')}</CardTitle>
              </CardHeader>
              <CardContent>
                <form onSubmit={handleSubmit} className="grid gap-4">
                  {selectedConflict && (
                    <div className="rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
                      {interpolate(t('classroomsPage.selectedTimeConflict'), {
                        time: `${selectedConflict.start_time}-${selectedConflict.end_time}`,
                  ...
                  <label className="space-y-1">
                    <span className="text-sm font-medium text-slate-700">
                      {t('classroomsPage.attendeesLabel')} <span className="text-red-500">*</span>
                    </span>
                    <Input
                      required
                      value={details.attendees}
                      placeholder={t('classroomsPage.attendeesPlaceholder')}
                  ...
                  <label className="space-y-1">
                    <span className="text-sm font-medium text-slate-700">
                      {t('classroomsPage.activityDescriptionLabel')} <span className="text-red-500">*</span>
                    </span>
                    <Textarea
                      required
                      rows={4}
                      value={details.activity_description}
                      placeholder={t('classroomsPage.activityDescriptionPlaceholder')}
                      onChange={(event) => setDetail('activity_description', event.target.value)}
                    />
                  </label>

                  <label className="space-y-1">
                    <span className="text-sm font-medium text-slate-700">{t('classroomsPage.equipmentNeedsLabel')}</span>
                    <Textarea
                      rows={3}
                      value={details.equipment_needs}
                      placeholder={t('classroomsPage.equipmentNeedsPlaceholder')}
                      onChange={(event) => setDetail('equipment_needs', event.target.value)}
                    />
                  </label>

                  <label className="space-y-1">
                    <span className="text-sm font-medium text-slate-700">{t('classroomsPage.specialRequestsLabel')}</span>
                    <Textarea
                      rows={3}
                      value={details.special_requests}
                      placeholder={t('classroomsPage.specialRequestsPlaceholder')}
                      onChange={(event) => setDetail('special_requests', event.target.value)}
                    />
                  </label>

                  <label className="space-y-1">
                    <span className="text-sm font-medium text-slate-700">{t('classroomsPage.additionalNotesLabel')}</span>
                    <Textarea
                      rows={3}
                      value={details.notes}
                      placeholder={t('classroomsPage.additionalNotesPlaceholder')}
                  ...
                  <div className="flex justify-end">
                    <Button type="submit" disabled={saving || Boolean(message) || Boolean(selectedConflict)}>
                      ...
                      {saving ? t('classroomsPage.submitting') : t('classroomsPage.submitDetail')}
                    </Button>
                  </div>
                </form>
              </CardContent>
