...
export function SchoolPolicyWorkspaceContent({
  embedded = false,
}: {
  embedded?: boolean;
}) {
  ...
  const text = locale.startsWith("zh")
    ? {
        title: "学校规章制度及退费规则",
        subtitle:
          "单独管理前台排课表下方展示的学校规则、退费说明和报名须知。支持中英法编辑和 AI 整理翻译。",
        save: "保存规章制度",
        saved: "学校规章制度已保存",
        loading: "正在加载学校规章制度...",
      }
    : locale.startsWith("fr")
      ? {
          ...
          subtitle:
            "Gerez les politiques affichees sur le site public, avec edition chinois, anglais et francais et assistance IA.",
          save: "Enregistrer",
      ...
      : {
          title: "School Policies and Refund Rules",
          subtitle:
            "Manage public school policies, refund rules, and registration notes with Chinese, English, and French AI support.",
          save: "Save Policies",
  ...
  useEffect(() => {
    ...
    settingsApi
      ...
      .catch((err) =>
        setError(
          err instanceof Error ? err.message : labels.settings.loadFailed,
        ),
      )
      .finally(() => setLoading(false));
  }, [router, locale, labels.settings.loadFailed]);

  function setPolicyField<K extends keyof SchoolPolicy>(
    targetLocale: PolicyLocale,
    key: K,
    value: SchoolPolicy[K],
  ) {
    setPolicies((current) => ({
      ...current,
      [targetLocale]: {
      ...
      drafts.forEach((draft) => {
        ...
        next[localeKey] = {
          ...
          ...(draft.fields.body_markdown
            ? { body_markdown: draft.fields.body_markdown }
            : {}),
            ...
            <CardContent className="flex flex-wrap items-center gap-2 py-4">
              ...
              {localeOptions.map((option) => (
                <Button
                  ...
                  type="button"
                  variant={
                    contentLocale === option.value ? "default" : "outline"
                  }
              ...
              <CardContent className="space-y-4">
                ...
                <label className="block space-y-1">
                  ...
                  <Input
                    ...
                    onChange={(event) =>
                      setPolicyField(contentLocale, "title", event.target.value)
                    }
                  />
                </label>
                <label className="block space-y-1">
                  <span className="text-sm font-medium">
                    {labels.settings.bodyMarkdown}
                  </span>
                  <Textarea
...
export default function AdminSchoolPolicyPage() {
  return <SchoolPolicyWorkspaceContent />;
}
